...
namespace util
{
		...
		, output_fptrA fptrErrorOnStringA  )
	{
		...
		LogF( L"%s", L"logger init" );
	}

	void Logger::LogMsg(wchar_t const * str, uint32 len)
	{
		if(0xffffffff == len)
			len = static_cast<uint32>( wcslen(str) );
		vLog (str, len);
	}
	void Logger::LogMsg(char const * str, uint32 len)
	{
		if(0xffffffff == len)
			len = static_cast<uint32>( strlen(str) );
		vLog (str, len);
	}
	...
	void Logger::LogString( wstring const & str )
	{
		if(!str.empty())
		{
			vLog(str.c_str(), static_cast<uint32>( str.length() ) );
			if(L'\n' != *str.rbegin())
				vLog(L"\n", 1);
		}
	}
	...
	void Logger::vInit( uint32 flag, wchar_t const * fname
	...
	void Logger::vLog( wchar_t const * str, uint32 len )
	{
	...
	void Logger::vLog( char const * str, uint32 len )
	{
		if (CHK_FLAGS (flag_, FLAG_OUTPUT))
