...
template <typename T>
class create_using_tls
{
public:
	static T* create()
	{
		__declspec(thread) static T* t;
		t = new T;
		return t;
	}
	static void destroy(T *p)
...
template <typename T, 
template <class> class creation_policy = create_using_static,
template <class> class life_time_policy = default_life_time,
template <class, class> class thread_policy = multi_thread,
