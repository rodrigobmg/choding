...
m_pDevice( NULL )
{
	LOG_INFO_F( "================ ResMgr Construct [0x%08x]" , this );
	Clear();
}

GdsResMgr::~GdsResMgr()
{
	LOG_INFO_F( "================ ResMgr Destruct [0x%08x]" , this );
	Release();
}
...
void GdsResMgr::ReleaseRes( const TCHAR* alias )
{
	...
	res_map_table_t::iterator itAll = m_LoadedResList.find( alias );
...
{
	res_map_table_t::iterator itAll = m_LoadedResList.find( filename );
	if ( itAll != m_LoadedResList.end() )
	{
		return itAll->second;
	}
...
bool GdsResMgr::stack_data_to_container( const TCHAR* filepath , GdsResBasePtr pres )
{
	...
	res_map_table_t::iterator it = m_LoadedResList.find( filepath );
...
void GdsResMgr::AllocIndexBuffer( LPDIRECT3DINDEXBUFFER9& pIB , uint32 size )
{
	if ( m_pDevice )
	{
...
void GdsResMgr::AllocVertexBuffer( LPDIRECT3DVERTEXBUFFER9& pVB , uint32 size )
{
	if ( m_pDevice )
	{
...
GdsComponent* GdsResMgr::AllocGameComponent()
{
	return NULL;
}

void GdsResMgr::FreeGameComponent( GdsComponent* pComponent )
{

}

int GdsResMgr::GetCountComponent()
{
	return 0;
}

GdsGameObject* GdsResMgr::AllocGameObject()
{
	GdsGameObject* object = new GdsGameObject;
	m_res_objects.push_back( object );
	return object;
}

void GdsResMgr::FreeGameObject( GdsGameObject* pObject )
{

}

int GdsResMgr::GetCountGameObject()
{
	return (int)m_res_objects.size();
}