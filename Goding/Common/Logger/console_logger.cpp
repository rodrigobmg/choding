...
namespace util
{
	namespace PRIVATE
	{
		static uint16 const COLOR[eConsolColor::NUM]	= {  
			0
			, FOREGROUND_BLUE
			, FOREGROUND_GREEN
			, FOREGROUND_RED
	...
	{
		vWriteMsg(msg.c_str(), (uint32)msg.length());
	}
	...
	void ConsoleLogger::WriteMsgFormatted( wchar_t const * fmt, ...)
	{
		...
		else
		{
			...
			const uint32 ERR_MSG_LEN = static_cast<uint32>( wcsnlen_s(ERR_MSG, 128) );
	...
	void ConsoleLogger::vWriteMsg(wchar_t const * msg, uint32 length)
	{
		vWriteDepth();
		DWORD writen = 0 ;
	...
	void ConsoleLogger::vWriteMsg(char const* msg, uint32 length)
	{
		vWriteDepth();
		DWORD writen = 0 ;
	...
	{
		vWriteMsg(block_name, static_cast<uint32>( wcslen(block_name)) );
		vWriteMsg(L"\n", 1U);
	...
	void ConsoleLogger::WriteMsg( wchar_t const * msg, uint32 len/*=0xffffffff*/ )
	{
		if(0xffffffff == len)
			len = static_cast<uint32>( wcslen(msg) );
		vWriteMsg(msg, len);
	}
	void ConsoleLogger::WriteMsg( char const * msg, uint32 len/*=0xffffffff*/ )
	{
		if(0xffffffff == len)
