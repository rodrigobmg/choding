...
namespace util
{
	namespace string
	{
		...
		bool mb2wc( wchar_t * buffer, const char * in, size_t bufferLength, eCodePage codepage )
		{
			...
			const uint32 FLAG = ( codepage== utf_8) ? MB_ERR_INVALID_CHARS : MB_PRECOMPOSED;
			const int32 NEED_LEN = ::MultiByteToWideChar(codepage, FLAG, in, -1, NULL, 0);

			ASSERT(0 != NEED_LEN);
			ASSERT(NEED_LEN <= (int32)bufferLength);
			if(0 == NEED_LEN || NEED_LEN > (int32)bufferLength)
				return false;

			wmemset(buffer, 0, NEED_LEN);

			int32 num = ::MultiByteToWideChar(codepage, FLAG, in, -1, buffer, (int32)bufferLength);
		...
		bool wc2mb( char * buffer, const wchar_t * in, size_t bufferLength, eCodePage codepage )
		{
			...
			const int32 NEED_LEN = WideCharToMultiByte(codepage, 0, in, -1, 0, 0, NULL, NULL);

			ASSERT(0 != NEED_LEN);
			ASSERT((int32)bufferLength >= NEED_LEN);
			if(0 == NEED_LEN || NEED_LEN > (int32)bufferLength)
				return false;

			memset(buffer, 0, NEED_LEN);

			int32 num = WideCharToMultiByte(codepage, 0, in, -1, buffer, (int32)bufferLength, NULL, NULL);
		...
		{
			uint32 const length = (uint32)(ws.length()*2);
		...
		std::wstring GetLastErrorString( wchar_t const * file/*=L"where"*/, uint32 line/*=0xffff*/ )
		{
